...
// Vider la liste exis)tante de triangles
app.triangles.clear();

// Créer un très grand triangle (-1000, -1000), (500, 3000), (1500, -1000)
Triangle grandTriangle = {{-1000, -1000},
                            {500, 3000},
                            {1500, -1000},
                            true};
...
for(int i =0; i<app.points.size(); i++){
    ...
    for(int j = 0; j<app.triangles.size(); j++){
        ...
        if(CircumCircle(app.points[j].x, app.points[j].x,
                        app.triangles[j].p1.x, app.triangles[j].p1.y, app.triangles[j].p2.x, app.triangles[j].p2.y, app.triangles[j].p3.x, app.triangles[j].p3.y,
                        &xc, &yc, &rsqr)){
            ...
            LS.push_back({app.triangles[j].p3,app.triangles[j].p1});
    ...
    for(int k = 0; k<LS.size(); k++){
        ...
        for(int m = 0; m<LS.size(); m++){
            if((LS[k].p1==LS[m].p2)&&(LS[k].p2==LS[m].p1)){
                // les virer;
                LS.erase(LS.begin()+k);
                LS.erase(LS.begin()+m);
            }   
    ...
    for(int l = 0; l<LS.size(); l++){
           ...
           app.triangles.push_back({LS[l].p1,LS[l].p2,app.points[l]});
    } 
}    
}
